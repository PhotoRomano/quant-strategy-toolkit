...
description: Design and execute a rigorous walk-forward validation for a quantitative trading strategy — prove it generalizes to unseen data, not just fits history. Use this whenever someone asks "does my strategy actually work out of sample?", "how do I split time-series data for backtesting?", "why did my backtest beat the market but live trading flopped?", "set up a walk-forward test", "expanding window validation", "is my strategy overfit?", "how should I structure in-sample vs out-of-sample periods?", or whenever someone wants to know if a strategy's backtest results are trustworthy before deploying real capital. Also invoke before optimizing any parameter, because parameter search without a held-out test set creates curve-fitting.
---
