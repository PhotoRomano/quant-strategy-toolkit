---
name: profit-ladder
description: Design and implement a tiered profit-taking ladder — a system that automatically sells a fixed fraction of a position when each gain threshold is crossed, fires each tranche exactly once per trade, and persists state so restarted processes never double-fire or silently skip. Use this whenever someone asks "how should I take partial profits?", "build a profit-taking ladder", "sell in tiers as it goes up", "take 25% off at +15%, another 25% at +30%...", "my profit taker fired twice on the same level", "position size is wrong after a partial fill", "cost basis is stale after I sold some shares", or whenever you see exit logic that sells a whole position at a single target — and the user wants to stay in the trade while locking in gains.
---
