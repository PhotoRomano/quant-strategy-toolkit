...
description: Design and implement a portfolio-level circuit breaker — a drawdown-triggered trading pause plus an EMA-based correlation gate that suppresses new entries in correlated or satellite assets when the lead asset's trend turns bearish. Use this whenever someone asks "how do I stop my system from digging a hole when things go wrong?", "should I add a kill switch to my bot?", "how do I pause trading after a bad stretch?", "my correlated alts blow up when BTC drops — how do I gate them?", "what is a correlation gate?", "protect my portfolio from tail risk", or "prevent the strategy from chasing entries in a downtrend". Also apply proactively before deploying any automated strategy live or when a backtest shows maximum drawdown exceeding the designer's stated pain threshold.
---
