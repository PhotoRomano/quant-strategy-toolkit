...
description: Build or audit a price-only market regime classifier that labels each decision date bull, neutral, or bear — using moving-average structure, golden/death cross, and multi-timeframe momentum, with optional hard-override gates for macro events that MAs cannot anticipate. Use this whenever someone asks "how do I detect the market regime?", "should I be buying or going defensive?", "how do I label each backtest date bull/bear?", "why does my regime signal lag?", "my classifier keeps calling bear during recoveries", "I want to reduce position size in downtrends", "build a regime filter for my strategy", or any question about conditioning strategy posture on market environment. Also use before wiring regime output to position sizing — see the companion regime-position-sizing skill.
---
