...
description: Audit a backtest universe for survivorship bias — the silent inflator that excludes bankruptcies, delistings, and index removals, making a strategy look better than it ever was. Use this whenever someone shares backtest results and asks "why does this crush in backtest but underperform live?", "is my universe sound?", "did I cherry-pick stocks?", "should I use Wikipedia's current S&P 500 list?", or whenever a strategy reports high win rates on a hand-curated or index-derived symbol list. Also use proactively before trusting any multi-year backtest, deploying a strategy live, or comparing backtest returns to a benchmark.
---
